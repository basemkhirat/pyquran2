from backend.quran_data import get_chapters, get_words_range, get_chapter_verse_count
...
class TestGetWordsRange:
    def test_fatiha_verse_1(self):
        words = get_words_range(1, 1, 1, 1)
    ...
    def test_fatiha_all_verses(self):
        words = get_words_range(1, 1, 1, 7)
        assert len(words) > 20

    def test_empty_range(self):
        words = get_words_range(1, 10, 1, 20)  # Fatiha only has 7 verses
        assert len(words) == 0
