...
class VADProcessor:
    ...
    Supports two modes:
    - **Legacy mode** (streaming_enabled=False): buffers until silence, returns segment.
    - **Streaming mode** (streaming_enabled=True): continuously accumulates audio,
      exposes it for periodic transcription, and signals speech end.
    """
    ...
    # ── Legacy mode (silence-based) ────────────────────────────────

    def process_chunk(self, pcm16_bytes: bytes) -> Optional[np.ndarray]:
        """Process an incoming PCM16 audio chunk (legacy silence-based mode).
